#!/bin/bash
# Optimizer search (README "Optimizer search") on the FLEURS-R NeuCodec STT pack —
# speech tokens -> text, 102 locales, packed by preparation/multipacking_fleurs.py.
#
# The STT pack's documents use tokens that do not exist in the TTS vocabulary
# (<|STT|> + one per locale), so the trainer appends them from
# fleurs/out/fleurs_stt_added_tokens.json in the order they were packed.
#
...
#
#   bash ablation-fleurs-stt.sh                        # full grid (16 runs)
#   bash ablation-fleurs-stt.sh --optimizers muon soap # subset
#   bash ablation-fleurs-stt.sh --dry-run              # print commands only
set -e
cd "$(dirname "$0")"
...
"$BASE/venv/bin/python" hyperparameter_search.py \
  --train-file "$BASE/fleurs/out/fleurs-stt" \
  --run-prefix fleurs-stt \
  --output-root "$BASE/runs/fleurs-stt" \
  --state-dir "$BASE/search_state/fleurs-stt" \
  ...
  --added-tokens-file "$BASE/fleurs/out/fleurs_stt_added_tokens.json" \
  "$@"
